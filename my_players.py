...
class PokerBot(Player):
    ...
    def evaluate_preflop(self, hole_cards):
        ...
        if is_pair:
            return 1.0 if ranks[0] >= 10 else 0.9 if ranks[0] >= 7 else 0.7  # Pocket pairs strong to weak
        if is_suited and is_connected:
            return 0.85 if ranks[0] >= 10 else 0.75  # High suited connectors
        if is_suited or is_connected:
            return 0.7 if ranks[0] >= 10 else 0.6  # High Suited or Connected Cards
        if ranks[0] >= 12:
            return 0.55  # High-card hands (AQ, KJ, etc.)
    ...
    def decide_action(self, game_state, action_history):
        ...
        elif game_state[8] > self.stack and strength > 0.5:
            return PlayerAction.ALL_IN, self.stack
        elif phase == 'river' and strength > 0.55:
            return PlayerAction.ALL_IN, self.stack
        elif phase == 'flop' and strength > 0.6:
            return PlayerAction.ALL_IN, self.stack
        elif phase == 'turn' and strength > 0.7:
            ...
            if opponent_tendency in ['aggressive', 'loose']:
                if self.stack > (self.initial_stack * 0.70):
                    if random.sample([0, 1], 1):
                        return PlayerAction.RAISE, min(random.randrange(100, self.stack), self.stack)
                    else:
                        return PlayerAction.RAISE, min(100, self.stack)
                else:
                    return PlayerAction.RAISE, min(100, self.stack)
